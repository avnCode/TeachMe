...
export default function QuizApp() {
    ...
    useEffect(() => {  
        ...
        localforage.getItem('teachme-topics').then((savedTopics) => {  
            if (savedTopics) {  
    ...
    useEffect(() => {  
        // Save topics to IndexedDB whenever topics state changes  
        localforage.setItem('teachme-topics', topics)  
    }, [topics])
    ...
    // Event handlers  
    const handleFile = async (file, setImage) => {  
...
useEffect(() => {  
    if (currentQuestion) {  
        setEditableQuestion(currentQuestion.question);  
        setEditableAnswer(currentQuestion.answer);  
    }  
}, [currentQuestion]);  
            ...
            <div className="max-w-xl mx-auto space-y-6">  
                ...
                {!currentQuestion && (  
                    <button  
                        ...
                        className="w-full bg-blue-600 text-white p-4 rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all transform hover:scale-[1.02]"  
                    >  
                        {!currentTopic  
                        ...
                        <div className="p-6 bg-gradient-to-r from-blue-50 to-blue-100">  
                            <div className="flex items-center justify-between mb-4">  
                                ...
                                <button  
                                    ...
                                    className="bg-yellow-600 text-white p-1 rounded font-medium hover:bg-yellow-700 transition-all transform hover:scale-[1.02] text-xs"  
                                >  
                                    Edit  
                                </button>  
                            </div>  
                        ...
                        <div className="p-6 space-y-4">  
                            ...
                            {isEditing ? (  
                                <div className="flex space-x-4">  
                                    <button  
                                        onClick={() => {  
                                            const updatedQuestion = { ...currentQuestion, question: editableQuestion, answer: editableAnswer };  
                                            // Update the topics object with the new question and answer  
                                            const updatedTopics = { ...topics };  
                                            const questionIndex = updatedTopics[currentTopic].findIndex(q => q.question === currentQuestion.question);  
                                            if (questionIndex !== -1) {  
                                                updatedTopics[currentTopic][questionIndex] = updatedQuestion;  
                                                setTopics(updatedTopics);  
                                            }  
                                            setIsEditing(false);  
                                            setCurrentQuestion(updatedQuestion);  
                                        }}  
                                        className="w-full bg-green-600 text-white p-3 rounded-lg font-medium hover:bg-green-700 transition-all transform hover:scale-[1.02]"  
                                    >  
                                        Save  
                                    </button>  
                                    <button  
                                        ...
                                        className="w-full bg-red-600 text-white p-3 rounded-lg font-medium hover:bg-red-700 transition-all transform hover:scale-[1.02]"  
                                    >  
                                        Cancel  
                                    </button>  
                                </div>  
                            ...
                            {showAnswer && (  
                                <div className="space-y-4">  
                                    <div className={`p-4 rounded-lg ${userAnswer.toLowerCase().trim() === currentQuestion.answer.toLowerCase().trim() ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>  
                                    ...
                                    <button  
                                        onClick={getRandomQuestion}  
                                        className="w-full bg-blue-600 text-white p-3 rounded-lg font-medium hover:bg-blue-700 transition-all transform hover:scale-[1.02] flex items-center justify-center gap-2"  
                                    >  
